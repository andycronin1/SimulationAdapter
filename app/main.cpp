...
// -------- Example logic to expose the adapter to a REST API from the web app -------- //

// void handleControlRequest(const httplib::Request& req, httplib::Response& res) {
//     // Parse the request (e.g., JSON input)
//     std::string command = req.get_param_value("command");
//     std::string sim = req.get_param_value("simulation");

//     // Route the command to the appropriate simulation
//     if (sim == "AirSim") {
//         // Call AirSim API
//     } else if (sim == "Bullet") {
//         // Call Bullet API
...
//     res.set_content("Command executed", "text/plain");
// }

// ------- Example logic to determine which simulation to use based on the request ------- //

// std::unordered_map<std::string, std::function<void()>> simulationHandlers;

// void setupSimulations() {
//     simulationHandlers["AirSim"] = sendCommandToAirSim;
//     simulationHandlers["Bullet"] = sendCommandToBullet;
// }

// void routeCommand(const std::string& sim) {
//     if (simulationHandlers.find(sim) != simulationHandlers.end()) {
//         simulationHandlers[sim](); // Call the appropriate handler
//     } else {
//         std::cerr << "Simulation not found: " << sim << std::endl;
//     }
...
int main() {
    ...
    // server.Post("/control", handleControlRequest);

    // std::cout << "Adapter running on port 8080..." << std::endl;
